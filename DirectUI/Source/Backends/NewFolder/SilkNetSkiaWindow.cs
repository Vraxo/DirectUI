...
namespace DirectUI.Backends.SkiaSharp
{
    public class SilkNetSkiaWindow : IDisposable
    {
            ...
            Vector2D<int>? initialPosition = null)
        {
            ...
            // Apply initial position before creation to avoid (0,0) flash
            ...
            // Prevent flicker by hiding until fully configured
        ...
        private void OnLoad(Action<UIContext> uiDrawCallback, Color4 backgroundColor)
        {
            ...
            Color4 finalBackgroundColor = useTransparentBg
            ...
            _gl.ClearColor(
                finalBackgroundColor.R,
                finalBackgroundColor.G,
                finalBackgroundColor.B,
                finalBackgroundColor.A);

            var glInterface = GRGlInterface.Create();
            _grContext = GRContext.CreateGl(glInterface);

            // Create initial Skia surface
        ...
        private void ApplyWindowStyles()
        {
            ...
            bool wantsModern = backdropType != WindowBackdropType.Default
                && OperatingSystem.IsWindowsVersionAtLeast(10, 0, 22621);

            if (OperatingSystem.IsWindowsVersionAtLeast(10, 0, 17763))
            {
                bool immersive = wantsModern || titleBarTheme != WindowTitleBarTheme.Default;
                if (immersive)
                {
                    int useDark = titleBarTheme == WindowTitleBarTheme.Light ? 0 : 1;
            ...
            if (wantsModern)
            {
                ...
                if (backdropType == WindowBackdropType.Mica
                    && titleBarTheme == WindowTitleBarTheme.Light)
                {
                    backdropValue = (int)DwmApi.DWMSYSTEMBACKDROP_TYPE.DWMSBT_TRANSIENTWINDOW;
                }
        ...
        private void OnKeyDown(IKeyboard kb, Key key, int sc) => Input.AddKeyPressed(MapKey(key));
        private void OnKeyUp(IKeyboard kb, Key key, int sc) => Input.AddKeyReleased(MapKey(key));
        ...
        private static MouseButton MapMouseButton(Silk.NET.Input.MouseButton button)
            => button switch
            {
                ...
                Silk.NET.Input.MouseButton.Button4 => MouseButton.XButton1,
                Silk.NET.Input.MouseButton.Button5 => MouseButton.XButton2,
                _ => MouseButton.Left,
            };
