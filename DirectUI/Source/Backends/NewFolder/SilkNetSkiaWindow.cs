...
public class SilkNetSkiaWindow : IDisposable
{
    ...
    public SilkNetSkiaWindow(string title, int width, int height, SilkNetWindowHost owner, bool isModal)
    {
        ...
        // Set IsVisible to false to prevent the window from appearing before it's fully configured and positioned.
        // This helps prevent a visual flicker where the window might appear at a default location for a frame.
    ...
    private void OnLoad(Action<UIContext> uiDrawCallback, Color4 backgroundColor)
    {
        ...
        bool useTransparentBg = _owner.BackdropType != WindowBackdropType.Default && OperatingSystem.IsWindowsVersionAtLeast(10, 0, 22621);
        Color4 finalBackgroundColor;

        if (useTransparentBg)
        {
            if (_owner.TitleBarTheme == WindowTitleBarTheme.Light)
        ...
        OnResize(IWindow.Size); // Create initial render target and surface
    ...
    public void Render()
    {
        if (IWindow.IsClosing || _skSurface is null || _renderer is null || _textService is null || _appEngine is null || _gl is null) return;
    ...
    private void ApplyWindowStyles()
    {
        ...
        if (OperatingSystem.IsWindowsVersionAtLeast(10, 0, 17763))
        {
            ...
            if (applyImmersiveTheme)
            {
                ...
                DwmApi.DwmSetWindowAttribute(Handle, DwmApi.DWMWINDOWATTRIBUTE.DWMWA_USE_IMMERSIVE_DARK_MODE, ref useDarkMode, sizeof(int));
            }
        }
        ...
        if (wantsModernBackdrop)
        {
            ...
            DwmApi.DwmSetWindowAttribute(Handle, DwmApi.DWMWINDOWATTRIBUTE.DWMWA_SYSTEMBACKDROP_TYPE, ref backdropValue, sizeof(int));
        }
    }
